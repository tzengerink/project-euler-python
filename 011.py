...
def diagonal_products(grid, n):
    """Get the products of N diagonal items each."""
    result = []
    for y in range(0, len(grid) - (n - 1)):
        for x in range(0, len(grid[y]) - (n - 1)):
            product = 1
            for i in range(n):
...
def horizontal_products(grid, n):
    """Get the products of N horizontal items each."""
    result = []
...
def max_product(grid, n):
    """Get the maximum product for any direction, taking N items each time."""
    return max([max(diagonal_products(grid, n)),
                max(horizontal_products(grid, n)),
                max(vertical_products(grid, n))
                ])
